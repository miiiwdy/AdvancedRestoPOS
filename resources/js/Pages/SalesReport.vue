...
<template>
    <div class="flex flex-row h-screen w-full bg-[#F8F8F8] tracking-tight overflow-y-hidden no-select">
        <div class="flex flex-col h-screen w-full px-4 gap-3">
            <div class="flex flex-row w-full pt-4 h-auto items-center justify-between">
                <div class="flex flex-row w-auto">
                    <a :href="route('pos')"
                        class="back-to-pos w-[3.2rem] h-[3.2rem] flex items-center justify-center rounded-full bg-white text-[#2D71F8] cursor-pointer">
                        <i class="ri-arrow-go-back-fill text-current text-xl"></i>
                    </a>
                    <div @click="openLogoutModal()"
                        class="flex w-auto pr-5 h-[3.2rem] rounded-full items-center bg-white ml-4 cursor-pointer">
                ...
                <div class="flex flex-row w-auto gap-4">
                    <div class="jam w-36 h-[3.2rem] rounded-full items-center bg-white flex">
                        <div class="wrap px-2 flex flex-row items-center justify-between w-full">
                            <p class="text-slate-700 font-semibold ml-3">{{ time }} <span class="text-slate-400">{{
                                period }}</span></p>
                            <div
            ...
            <div class="flex flex-row justify-between w-full h-14 rounded-2xl bg-white">
                <div class="flex items-center justify-center gap-3">
                    ...
                    <div @click="enableThisMonthSorting()"
                        class="flex items-center justify-between h-[70%] w-32 rounded-full border border-slate-300 bg-transparent cursor-pointer">
                    ...
                    <div @click="enableThreeMonthSorting()"
                        class="flex items-center justify-between h-[70%] w-32 rounded-full border border-slate-300 bg-transparent cursor-pointer">
                    ...
                    <div @click="enableSixMonthSorting()"
                        class="flex items-center justify-between h-[70%] w-32 rounded-full border border-slate-300 bg-transparent cursor-pointer">
                    ...
                    <div @click="enableOneYearSorting()"
                        class="flex items-center justify-between h-[70%] w-24 rounded-full border border-slate-300 bg-transparent cursor-pointer">
                ...
                <div class="flex items-center justify-center gap-3 pr-2">
                    <div class="text-sm font-medium text-slate-700 pl-3">{{ namaResto }} | <span
                            class="text-gray-500">{{ namaOutlet }}</span></div>
                    <div
                        class="icon w-7 h-7 bg-[#f0f7ff] rounded-full flex items-center justify-center text-[#2D71F8] mr-1">
            ...
            <div class="sales-data-cards">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 w-full">
                    <div class="flex flex-col h-full rounded-3xl bg-white">
                        <div
                            class="w-full h-[72%] pt-3 px-3 flex flex-col justify-between items-center border-b border-neutral-100 pb-3">
                            <div class="flex flex-row justify-start items-center w-full h-20">
                                <div
                                    class="icon w-9 h-9 bg-[#f0f7ff] rounded-full flex items-center justify-center text-[#2D71F8] mr-3">
                            ...
                            <div class="flex flex-row justify-between items-end w-full h-full">
                                <div class="text-2xl md:text-3xl font-medium text-slate-700 shimmer">
                                    {{ totalSalesAmount.toLocaleString('id-ID') }}</div>
                                <div class="text-md text-gray-400 font-medium">IDR</div>
                            </div>
                        </div>
                        <div class="flex flex-row w-full h-[27%] justify-between items-center px-3">
                            <div v-if="p_salesAmount"
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#fff5f5] rounded-full">
                                <div class="text-xs font-medium text-[#FC4A4A]">-IDR
                                    {{ salesDifference.toLocaleString('id-ID') }}</div>
                            </div>
                            <div v-else
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#f2fff9] rounded-full">
                                <div class="text-xs font-medium text-[#1C8370]">+IDR
                                    {{ salesDifference.toLocaleString('id-ID') }}</div>
                            </div>

                            <div v-if="p_salesAmount"
                                class="flex items-center justify-center h-[68%] w-auto pl-3 pr-1 bg-[#fff5f5] rounded-full">
                    ...
                    <div class="flex flex-col h-full rounded-3xl bg-white">
                        <div
                            ...
                            <div class="flex flex-row justify-between items-end w-full h-full">
                                <div class="text-2xl md:text-3xl font-medium text-slate-700 shimmer">{{
                                    totalProductSales.toLocaleString('id-ID') }}</div>
                                <div class="text-md text-gray-400 font-medium">Items</div>
                            </div>
                        </div>
                        <div class="flex flex-row w-full h-[27%] justify-between items-center px-3">
                            <div v-if="p_productSales"
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#fff5f5] rounded-full">
                                <div class="text-xs font-medium text-[#FC4A4A]">-{{
                                    productDifference.toLocaleString('id-ID') }} Items</div>
                            </div>
                            <div v-else
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#f2fff9] rounded-full">
                                <div class="text-xs font-medium text-[#1C8370]">+{{
                                    productDifference.toLocaleString('id-ID') }} Items</div>
                            </div>
                            <div v-if="p_productSales"
                                class="flex items-center justify-center h-[68%] w-auto pl-3 pr-1 bg-[#fff5f5] rounded-full">
                    ...
                    <div class="flex flex-col h-full rounded-3xl bg-white">
                        <div
                            ...
                            <div class="flex flex-row justify-between items-end w-full h-full">
                                <div class="text-2xl md:text-3xl font-medium text-slate-700 shimmer">{{
                                    netProfits.toLocaleString('id-ID') }}</div>
                                <div class="text-md text-gray-400 font-medium">IDR</div>
                            </div>
                        </div>
                        <div class="flex flex-row w-full h-[27%] justify-between items-center px-3">
                            <div v-if="p_netProfits"
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#fff5f5] rounded-full">
                                <div class="text-xs font-medium text-[#FC4A4A]">-IDR {{
                                    profitDifference.toLocaleString('id-ID') }}</div>
                            </div>
                            <div v-else
                                class="flex items-center justify-center h-[68%] w-auto px-3 bg-[#f2fff9] rounded-full">
                                <div class="text-xs font-medium text-[#1C8370]">+IDR {{
                ...
                <div class="flex w-full flex-col lg:flex-row gap-4 h-auto">
                    <div class="fav-product flex flex-col w-full lg:w-[30%] bg-white rounded-3xl px-4 h-auto">
                        ...
                        <div class="flex flex-row w-full h-11 gap-2 mb-3">
                            <div
                                class="image-label text-xs md:text-sm flex items-center justify-center px-3 md:px-5 rounded-2xl text-slate-500 border border-grey-400">
                                Img</div>
                            <div
                                class="image-label text-xs md:text-sm flex-1 flex items-center justify-center rounded-2xl text-slate-500 border border-grey-400">
                                Product Name</div>
                            <div
                                class="image-label text-xs md:text-sm flex items-center justify-center px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Orders</div>
                        </div>

                        <div
                            class="flex flex-col w-full overflow-y-auto h-auto xl:h-[24rem] lg:h-[28rem] space-y-3 py-2">
                            <div v-for="product in sortedProductsByQuantity" :key="product.kode_product"
                                ...
                                <div
                                    class="p_image flex items-center justify-center bg-zinc-100 w-10 h-10 md:w-14 md:h-14 rounded-xl">
                                    <img class="max-h-10 w-auto object-contain"
                                        :src="'http://127.0.0.1:8000/storage/' + product.foto_product" alt="" />
                                </div>
                                <div class="flex flex-col items-start justify-center flex-1 px-3">
                                    <div class="text-xs md:text-sm font-medium text-slate-700 pb-1 truncate w-full">{{
                                        product.nama_product }}</div>
                    ...
                    <div
                        class="all-order flex flex-col h-auto lg:h-[28rem] xl:h-[24rem] w-full lg:w-[70%] bg-white rounded-3xl px-4">
                        <div class="flex w-full justify-between h-56 items-center flex-row pl-3">
                            ...
                            <div class="flex flex-row justify-center items-center mr-5">
                                <div class="flex flex-row h-[70%] w-full">
                                    <div
                                        class="flex items-center w-full pl-1 h-[60%] bg-white rounded-full mx-4 border">
                                        <div
                                            class="icon flex items-center justify-center w-8 h-8 bg-gray-100 rounded-full shrink-0 text-gray-700">
                                            <i class="bi bi-search text-sm text-current"></i>
                                        </div>
                                        <input v-model="searchQuery" type="text"
                                            class="flex-1 bg-transparent px-4 placeholder:text-gray-400 text-gray-700 font-semibold border-none focus:outline-none focus:ring-0"
                        ...
                        <div class="flex flex-row w-full h-36 gap-2 mb-3 overflow-x-auto pb-2">
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-16 md:min-w-24 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                #</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-14 md:min-w-18 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Order</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-24 md:min-w-36 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Cashier</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-28 md:min-w-40 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Date & Time</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-16 md:min-w-24 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Guests</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-32 md:min-w-44 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Total Payment</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-16 md:min-w-24 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Status</div>
                            <div
                                class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-20 md:min-w-28 px-2 md:px-4 rounded-2xl text-slate-500 border border-grey-400">
                                Detail</div>
                        </div>
                        ...
                        <div class="flex flex-col w-full h-auto max-h-64 md:max-h-[20rem] overflow-y-auto">
                            <div class="w-full overflow-x-auto">
                                ...
                                <div v-for="(order, index) in filteredOrderData" :key="order.id"
                                    class="flex flex-row w-full h-10 md:h-[2.5rem] gap-2 mb-3">
                                    <div
                                        class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-16 md:min-w-24 px-2 md:px-4 rounded-2xl text-slate-700 font-medium">
                                        #{{ order.order_id }}
                                    </div>
                                    <div
                                        class="label text-[0.6rem] md:text-[0.73rem] flex-shrink-0 flex items-center justify-center min-w-14 md:w-18 xl:w-20 text-center px-2 md:px-4 rounded-2xl bg-[#f0f7ff] text-[#2D71F8] font-medium">
                                        {{ order.order_type || 'N/A' }}
                                    </div>
                                    <div
                                        class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-24 md:min-w-36 px-2 md:px-4 rounded-2xl text-slate-700 font-medium truncate">
                                        {{ order.nama_kasir || 'N/A' }}
                                    </div>
                                    <div
                                        class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-28 md:min-w-40 px-2 md:px-4 rounded-2xl text-slate-700 font-medium">
                                    ...
                                    <div
                                        class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-32 md:min-w-44 px-2 md:px-4 rounded-2xl text-slate-700 font-medium">
                                    ...
                                    <div @click="openOrderDetailModal(order)"
                                        class="label text-xs md:text-sm flex-shrink-0 flex items-center justify-center min-w-20 md:min-w-28 px-2 md:px-4 rounded-2xl text-[#2D71F8] underline font-medium cursor-pointer">
                                        Detail
                                    </div>
                                </div>
                            </div>
            ...
            <div v-if="isOrderDetailModalOpen"
                class="fixed inset-0 flex items-center justify-center bg-slate-400 bg-opacity-50"
                @click.self="closeOrderDetailModal">
                <div
                    class="absolute top-1/2 left-1/3 transform -translate-x-[2rem] -translate-y-1/2 bg-white rounded-2xl w-[35rem] shadow-2xl">
                    <div
                        class="flex flex-row justify-between items-center justify-center shadow-lg shadow-gray-100 rounded-md p-3 pb-3">
                        <div class="flex w-9 h-9 bg-white"></div>
                        <div class="flex flex-col justify-center items-center">
                            <h2 class="text-normal font-normal">Order Detail</h2>
                        </div>
                        <div class="flex items-center justify-center rounded-full bg-[#fff2f3] w-9 h-9 text-[#FC4A4A] cursor-pointer"
                    ...
                    <div class="flex w-full flex-col max-h-[34rem] overflow-auto mb-5">
                        <div class="flex flex-col w-full h-50 py-4 mb-1 px-5 justify-between">
                            <div class="flex flex-row justify-left items-center w-full h-auto mb-2">
                                <div class="text-xl italic font-medium mr-11 text-gray-800">#{{ selectedOrder?.order_id
                                }}</div>
                                <div
                                    class="flex justify-center items-center bg-[#f2fff9] text-sm text-[#1C8370] rounded-full py-[0.100rem] px-4">
                                    All Done</div>
                            </div>
                            <div class="flex flex-row w-auto pb-4 border-b-2 border-dashed text-gray-500">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <div class="flex justify-center items-start text-[0.920rem]">{{
                                        formattedDataOrderUpdatedAt }}</div>
                                </div>
                            </div>
                            <div
                                class="flex flex-col justify-start items-start text-gray-500 w-full h-auto pt-3 pb-4 border-dashed border-b-2">
                                <div class="text-[0.920rem]">Total Price: Rp{{
                                    formatCurrency(selectedOrder?.total_harga_after_all) }}</div>
                                <div class="text-[0.920rem]">Rounding: {{ selectedOrder?.rounding }}</div>
                                <div class="text-[0.920rem]">Amount Paid: Rp{{
                                    formatCurrency(selectedOrder?.amount_paid) }}</div>
                        ...
                        <div class="flex w-full h-auto flex-col">
                            <div
                                class="flex flex-col max-h-40 w-full mb-4 pb-4 overflow-y-auto shadow-lg shadow-gray-100 rounded-md">
                                <div v-for="product in selectedProducts" :key="product.id"
                                    class="text-[0.940rem] text-gray-500 px-5 mb-2">
        ...
        <div v-if="isLogoutModalOpen" class="fixed inset-0 flex items-center justify-center bg-slate-400 bg-opacity-50"
            @click.self="closeLogoutModal">
            <div
                class="absolute top-1/2 left-1/3 transform -translate-x-[2rem] -translate-y-1/2 bg-white rounded-2xl w-[31rem] shadow-2xl">
                <div
                    class="flex flex-row justify-between items-center justify-center shadow-lg shadow-gray-100 rounded-md p-3 pb-3">
                    <div class="flex w-9 h-9 bg-white"></div>
                ...
                <div class="flex w-full items-center justify-center flex-col px-3 max-h-[30rem] overflow-auto mb-5">
                    ...
                    <div
                        class="jam w-52 h-[3.5rem] rounded-full items-center bg-white flex border border-slate-200 mb-7">
                        <div class="wrap relative w-full px-2 flex items-center justify-start">
                            ...
                            <div class="icon bounce-left-right h-9 bg-[#FC4A4A] rounded-full flex items-center justify-center text-white cursor-pointer absolute right-2 -top-1/4  z-10 transition-[width] duration-300 ease-in-out"
                                ref="logoutBtn" :style="{ width: `${iconWidth}px` }" @pointerdown="startDrag"
